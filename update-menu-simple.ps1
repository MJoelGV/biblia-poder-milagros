...
foreach ($file in $htmlFiles) {
    ...
    try {
        ...
        if ($content -match 'id=["'']side-menu["''][\s\S]*?<div[^>]*class=["'']overlay["'']') {
            # Reemplazar el menú existente
            $content = $content -replace '(<!-- Menú lateral -->[\s\S]*?<div[^>]*class=["'']overlay["''][^>]*>)', "`$1$menuHTML"
            $updated = $true
        }
        
        # Asegurarse de que el script del menú esté presente
        if ($content -notmatch 'function\s+toggleMenu\s*\(') {
            $content = $content -replace '(</body>)', "<script>$menuJS</script>`n`$1"
            $updated = $true
        }
        
        # Asegurarse de que el CSS del menú esté incluido
        if ($content -notmatch 'menu-styles\.css') {
            $content = $content -replace '(</head>)', "    <link rel='stylesheet' href='css/menu-styles.css'>`n`$1"
            $updated = $true
        }
        
        # Asegurarse de que Font Awesome esté incluido
        if ($content -notmatch 'font-awesome') {
            $content = $content -replace '(</head>)', "    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css'>`n`$1"
            $updated = $true
        }
